          ...
          <table id="usersTable" class="table table-striped" style="width:100%">
            ...
            <tbody>
              @foreach ($users as $user)
                <tr>
                  <td>{{ $user->id }}</td>
                  <td><a href="{{ route('users.show', $user->id) }}" target="_blank" class="profile-link"><img src="{{ asset($user->image->url) }}" class="rounded-circle user-profile-image" alt="Profile image"> {{ $user->name }} <i class="bi bi-box-arrow-up-right ms-1"></i></a></td>
                  <td>{{ $user->email }}</td>
