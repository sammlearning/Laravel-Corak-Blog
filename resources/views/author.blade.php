...
@section('content')
  <div class="row">
    <div class="col-lg-8">
      <h5 class="mb-3">{{ $user->posts->count() }} published {{ $user->posts->count() > 1 ? 'posts' : 'post' }} by the author <b><img src="{{ asset( $user->image->url_sm ) }}" class="rounded-circle" width="25"> {{ $user->name }}</b></h5>
